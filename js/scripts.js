class Calculator {
    ...
    resolution() {
      ...
      for(let i = 0; i <= upperValueArray.length; i++, console.log("loop")) {
  
        let operation = 0;
        let actualItem = upperValueArray[i];
  
        // faz a multiplicação
        if(actualItem == "x", console.log("multiplicação")) {
          result = calc.multiplication(upperValueArray[i - 1], upperValueArray[i + 1]);
          console.log(upperValueArray[i - 1] + "+" + upperValueArray[i + 1] + " - Resultado:" + result);
          operation = 1;
        // faz a divisão
        } else if(actualItem == "/") {
